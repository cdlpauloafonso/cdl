...
export default function AssocieSeCadastroPage() {
  ...
  const handleCnpjChange = (raw: string) => {
    ...
    else if (digits.length <= 12) value = `${digits.slice(0, 2)}.${digits.slice(2, 5)}.${digits.slice(5, 8)}/${digits.slice(8)}`;
    else value = `${digits.slice(0, 2)}.${digits.slice(2, 5)}.${digits.slice(5, 8)}/${digits.slice(8, 12)}-${digits.slice(12)}`;
  ...
  return (
    <div className="py-12 sm:py-16 bg-gradient-to-b from-white to-cdl-gray/30">
      <div className="container-cdl max-w-4xl">
        <Link href="/associe-se" className="text-sm text-cdl-blue hover:underline mb-4 inline-block">
        ...
        {done ? (
          <div className="mt-8 rounded-xl border border-green-200 bg-green-50 p-6">
            ...
            <Link href="/associe-se" className="btn-primary mt-4 inline-block">
              Voltar
            </Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="mt-6 bg-white rounded-xl border border-gray-200 p-6 space-y-6">
            <div>
