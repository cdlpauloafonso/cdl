...
import { getHistoriaById, listHistorias, createHistoria, updateHistoria, type HistoriaItemFirestore, type NewsLink } from '@/lib/firestore';
...
export default function AdminHistoriaEditPage() {
  const params = useParams();
  const router = useRouter();
  const id = params.id as string;
  const isNew = id === 'nova';
  const [news, setNews] = useState<Partial<HistoriaItemFirestore>>({
    title: '',
    slug: '',
    excerpt: '',
    content: '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload: HistoriaItemFirestore = {
      title: news.title!,
      slug: news.slug!,
    ...
    try {
      ...
      if (isNew) {
        ...
        await updateHistoria(id, payload);
      }
      ...
      // Histórias usam Firestore (não API); mensagens comuns de erro
      let message = raw;
      if (raw === 'API não configurada') {
        message = 'As histórias usam Firebase. Verifique se o Firebase está configurado e se você está logado como administrador.';
      } else if (raw.includes('permission') || raw.includes('Permission')) {
        message = 'Sem permissão. Verifique se está logado como administrador e se as regras do Firestore permitem escrita na coleção `historias`.';
      }
  ...
  async function shareHistoria() {
    const slug = (news.slug ?? '').trim();
    if (!slug) {
      alert('Defina o slug da história para compartilhar.');
      return;
    }
    setSharing(true);
    try {
      const shareUrl = `${window.location.origin}/historias/${encodeURIComponent(slug)}`;
      const shareText = `${news.title ?? 'História CDL'}\n${shareUrl}`;

      if (navigator.share) {
        await navigator.share({
          title: news.title ?? 'História CDL',
      ...
      if (navigator.clipboard?.writeText) {
        ...
        alert('Link da história copiado para a área de transferência.');
        return;
      }

      const ok = window.prompt('Copie o link da história:', shareUrl);
      if (ok !== null) {
        // sem-op: prompt já mostra o link para cópia manual
      }
    } catch {
      alert('Não foi possível compartilhar a história agora.');
    } finally {
      setSharing(false);
    }
  }
  ...
  return (
    <div>
      <Link href="/admin/historias" className="text-sm text-cdl-blue hover:underline mb-4 inline-block">← Histórias</Link>
      <h1 className="text-2xl font-bold text-gray-900">{isNew ? 'Nova história' : 'Editar história'}</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-base font-semibold text-gray-900">História salva com sucesso</h3>
                <p className="mt-1 text-sm text-cdl-gray-text">Redirecionando para a lista de histórias...</p>
              </div>
            </div>
          </div>
        </div>
