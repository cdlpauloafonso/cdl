...
/** IDs de parceiros (benefícios) para generateStaticParams (output: export). */
export async function listBeneficiosParceirosIdsAtBuild(): Promise<string[]> {
  if (typeof window !== 'undefined') return [];
  try {
    const admin = await import('firebase-admin');
    if (!admin.apps.length) {
    ...
    const db = admin.firestore();
    const snap = await db.collection('beneficiosParceiros').get();
    return snap.docs.map((d) => d.id);
  } catch {
    return [];
  }
...
/** IDs de pontos turísticos (Nossa Cidade CMS) para generateStaticParams (output: export). */
export async function listNossaCidadePontoIdsAtBuild(): Promise<string[]> {
  if (typeof window !== 'undefined') return [];
  try {
    const admin = await import('firebase-admin');
    if (!admin.apps.length) {
    ...
    const db = admin.firestore();
    const snap = await db.collection('nossaCidade').doc('main').get();
    if (!snap.exists) return [];
    const raw = snap.data()?.pontosTuristicos;
    if (!Array.isArray(raw)) return [];
    return raw
      .map((p) => (p && typeof p === 'object' && typeof (p as { id?: string }).id === 'string' ? (p as { id: string }).id : ''))
      .filter(Boolean);
  } catch {
    return [];
  }
